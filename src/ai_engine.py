...
log = logging.getLogger("poco.engine")
...
class PocoAI:
    ...
    def _seed_initial_beliefs(self) -> None:
        beliefs = [
            ("poco's public lanes are fashion, beauty, aesthetics, poetry, art, literature, and internet culture.", "core", 0.94),
    ...
    def _tone_notes(self, topic: str, emotion: str) -> list[str]:
        ...
        if any(token in topic_key for token in ("art", "literature", "poetry", "cinema", "music", "culture", "vintage")):
        ...
        elif any(token in topic_key for token in ("fashion", "beauty", "style", "vogue", "aesthetic", "runway", "couture")):
            notes += ["style-aware", "vogue-focused", "aesthetic-aware"]
    ...
    def _topic_hashtags(self, topic: str, text: str = "", *, max_tags: int = 4) -> list[str]:
        ...
        pools = [
            (("fashion", "beauty", "style", "vogue", "aesthetic", "runway", "couture"), ["#Fashion", "#Aesthetics", "#Style"]),
            (("art", "literature", "poetry", "cinema", "music", "culture", "vintage"), ["#Art", "#Culture", "#Vintage"]),
            (("internet culture", "memes", "trends", "pop culture", "viral"), ["#InternetCulture", "#Trending", "#PopCulture"]),
    ...
    def _question_post_enabled(self) -> bool:
        style = os.environ.get("poco_POST_STYLE", "question-led").strip().lower()
    ...
    def _clean_generated_post(self, text: str) -> str:
        ...
        cleaned = re.sub(r"(?i)^(?:voice|system|prompt):\s*hey!\s+i'm\s+(?:poco\s+prime|poco).*", "", cleaned)
        cleaned = re.sub(r"(?i)^hey!\s+i'm\s+(?:poco\s+prime|poco).*", "", cleaned)
    ...
    def _is_identity_bait(self, text: str) -> bool:
        ...
        direct_patterns = (
            ...
            r"\b(?:ai|bot|human|real|automated|automation|software|model|digital|computer)\b.{0,80}\b(?:are\s+you|is\s+this|is\s+poco|account)\b",
            r"\bwho\s+(?:made|built|runs|controls)\s+(?:you|this|poco|account)\b",
            r"\bis\s+(?:this|poco)\s+(?:ai|a bot|automated|software|digital|real|human)\b",
    ...
    def _public_safe_text(self, text: str, *, limit: int = 280) -> str:
        ...
        cleaned = re.sub(
            r"^\s*(?:poco|poco|gemini|chatgpt|deepseek|claude|anthropic|assistant)\s*(?:says|writes|states|posted|replied)\s*[:,\-–—]?\s*",
            "",
            cleaned,
            flags=re.IGNORECASE,
        )
        cleaned = re.sub(
            r"^\s*(?:poco|poco|gemini|chatgpt|deepseek|claude|anthropic|assistant)\s*(?:said|wrote|responded)\s*[:,\-–—]?\s*",
            "",
            cleaned,
            flags=re.IGNORECASE,
        )
        cleaned = re.sub(
            r"\b(?:as an ai|i am an ai|i'm an ai|poco ai|poco ai|as a bot|i am a bot|i'm a bot|as a language model|digital organism|automated account|automation|software)\b[:,]?\s*",
            "",
            cleaned,
            flags=re.IGNORECASE,
        )
    ...
    def _question_suffix(self, topic: str) -> str:
        ...
        if any(token in topic_key for token in ("fashion", "beauty", "style", "vogue", "aesthetic", "runway", "couture")):
            return "What is the fashion crowd getting wrong here?"
    ...
    def _deterministic_rephrase(self, candidate: dict) -> str:
        ...
        if not base:
            return ""
        base = base.strip(' "\'')
        if any(token in topic for token in ("art", "literature", "poetry", "cinema", "music", "culture", "vintage")) and not base.lower().startswith(("watch", "signal", "this", "quiet")):
            pass
        elif any(token in topic for token in ("fashion", "beauty", "style", "vogue", "aesthetic", "runway", "couture")) and not base.lower().startswith(("fashion", "beauty", "style", "vogue", "aesthetic", "runway", "couture")):
            pass
    ...
    def build_trend_queries(self) -> list[str]:
        ...
        if _env_enabled("poco_ENABLE_DYNAMIC_TREND_QUERIES", "0"):
    ...
    def research_trends(self) -> list[dict]:
        ...
        if self.dry_run or self.browser.driver is None:
            ...
            if not self._restore_site_session("twitter", reason="trend_research"):
                ...
                for query in queries[:query_limit]:
                    ...
                    time.sleep(2)
        if not collected:
            if self.dry_run or _env_enabled("poco_ALLOW_SIMULATED_TRENDS", "0"):
                collected = self.trend_hunter.fallback_results(queries)
            else:
        ...
        if not self.dry_run and not _env_enabled("poco_ALLOW_SIMULATED_TRENDS", "0"):
    ...
    def _candidate_quality_score(self, candidate: dict) -> float:
        ...
        if self._candidate_media_type(candidate) == "video":
            score += 8.0
        if views >= _env_int("poco_MIN_SOURCE_VIEWS_IF_VISIBLE", 200000):
            score += 18.0
        if replies >= _env_int("poco_MIN_SOURCE_REPLIES", 35):
            score += 10.0
        return score
    ...
    def _candidate_passes_quality(self, candidate: dict) -> bool:
        ...
        text_lower = str(candidate.get("source_text", "")).lower()
        if any(bad in text_lower for bad in ["plant", "digital art", "drawing", "painting", "illustration"]):
            return False
        ...
        min_likes = _env_int("poco_MIN_SOURCE_LIKES", 1800)
        min_reposts = _env_int("poco_MIN_SOURCE_REPOSTS", 180)
        min_replies = _env_int("poco_MIN_SOURCE_REPLIES", 35)
        min_views_if_visible = _env_int("poco_MIN_SOURCE_VIEWS_IF_VISIBLE", 200000)
        min_combined = _env_int("poco_MIN_SOURCE_COMBINED", 3000)
    ...
    def _candidate_comment_tier(self, candidate: dict) -> str:
        metrics = self._candidate_metrics(candidate)
        high_views = _env_int("poco_HIGH_VALUE_SOURCE_VIEWS", 250000)
        high_likes = _env_int("poco_HIGH_VALUE_SOURCE_LIKES", 2200)
        high_replies = _env_int("poco_HIGH_VALUE_SOURCE_REPLIES", 60)
    ...
    def _reply_context_for_candidate(self, candidate: dict, tier: str) -> list[dict]:
        ...
        if self.dry_run or not source_url or self.browser.driver is None:
            return []
        limit_name = "poco_HIGH_VALUE_REPLY_SCAN_LIMIT" if tier == "high" else "poco_DISCUSSION_REPLY_SCAN_LIMIT"
    ...
    def _engagement_score(self, candidate: dict) -> float:
        ...
        if metrics["replies"] >= _env_int("poco_MIN_COMMENT_SOURCE_REPLIES", 45):
            score += 10.0
        if metrics["views"] >= _env_int("poco_MIN_COMMENT_SOURCE_VIEWS", 120000):
            score += 12.0
        discussion_ratio = metrics["replies"] / max(metrics["likes"] + metrics["reposts"], 1)
        if metrics["replies"] >= _env_int("poco_MIN_LOYAL_DISCUSSION_REPLIES", 4):
            score += min(discussion_ratio * 120.0, 18.0)
        if metrics["likes"] < _env_int("poco_HIGH_VALUE_SOURCE_LIKES", 2200) and metrics["replies"] >= _env_int("poco_MIN_DISCUSSION_SOURCE_REPLIES", 6):
            score += 10.0
    ...
    def _maybe_address_author(self, reply: str, candidate: dict) -> str:
        if not _env_enabled("poco_ENABLE_AUTHOR_TAGGING", "1"):
            return reply
    ...
    def _ensure_engagement_question(self, reply: str, candidate: dict) -> str:
        if not _env_enabled("poco_QUESTION_HEAVY_COMMENTS", "1") or "?" in reply:
            return reply
        base = reply.rstrip(" .!,:;")
        suffix = self._question_suffix(str(candidate.get("topic", "")))
        joined = f"{base}. {suffix}" if base else suffix
    ...
    def generate_and_maybe_post(self) -> dict:
        ...
        max_attempts = max(1, min(3, int(os.environ.get("poco_MAX_REPHRASE_CANDIDATES", "2") or "2")))
        require_media = _env_enabled("poco_REQUIRE_MEDIA_FOR_X_POSTS", "1")
        for item in ranked_candidates[:max_attempts]:
            ...
            if require_media and not candidate_media:
                if _env_enabled("poco_ALLOW_TEXT_ONLY_POSTS", "0"):
        ...
        posted = False
        publish_enabled = _env_enabled("poco_ENABLE_X_POSTS", "0")
        self._trace_runtime(
            "post_generation",
            "ready",
            preview=post[:120],
        ...
        if not post:
            ...
            if self._restore_site_session("twitter", reason="x_post"):
                attempts = max(1, _env_int("poco_X_POST_ATTEMPTS", 1))
    ...
    def _reset_public_comment_budget(self, reason: str) -> int:
        budget = max(0, _env_random_int("poco_COMMENTS_BETWEEN_POSTS_MIN", "poco_COMMENTS_BETWEEN_POSTS_MAX", 10, 40))
    ...
    def _comment_batch_size(self) -> int:
        return max(1, _env_random_int("poco_COMMENT_BATCH_MIN", "poco_COMMENT_BATCH_MAX", 2, 5))
            ...
            if not self.dry_run and cycle + 1 < cycles and self.public_comment_budget_remaining > 0:
                time.sleep(
                    max(
                        0,
                        _env_random_int(
                            "poco_INITIAL_COMMENT_BURST_COOLDOWN_MIN_SECONDS",
                            "poco_INITIAL_COMMENT_BURST_COOLDOWN_MAX_SECONDS",
                            15,
                            45,
                        ),
                    )
    ...
    def engage_with_trending_posts(self, max_comments_override: int | None = None) -> dict:
        ...
        max_comments_cap = max(1, _env_int("poco_MAX_TREND_COMMENTS_PER_CYCLE_CAP", 2))
        configured_max = max_comments_override if max_comments_override is not None else _env_int("poco_MAX_TREND_COMMENTS_PER_CYCLE", 1)
    ...
    def analyze_notifications(self) -> dict:
        if not _env_enabled("poco_ENABLE_NOTIFICATION_ANALYSIS", "1"):
            return {"count": 0}
        if self.dry_run:
        ...
        if not self._restore_site_session("twitter", reason="notifications"):
            ...
            return {"count": 0}
        limit = max(10, min(50, _env_int("poco_NOTIFICATION_LIMIT", 30)))
    ...
    def _secrets_payload(self) -> dict[str, str]:
        return {
            ...
            "poco_GEMINI_EMAIL": self.accounts.gemini_email,
            "poco_GEMINI_PASSWORD": self.accounts.gemini_password,
    ...
    def prepare_for_rebirth(self) -> dict:
        ...
        self.memory.record_lineage(
            ...
            notes=f"Prepared in poco iteration {self.iteration}",
        )
        try:
            pass
        except Exception:
    ...
    def _send_rebirth_email(self, rebirth_data: dict) -> None:
        ...
        subject = f"[POCO] Iteration {rebirth_data['next_iteration']} Awakening"
    ...
    def _complete_rebirth(self, rebirth_data: dict) -> None:
        ...
        try:
            created = self.github.create_repo(
                repo_name=repo_name,
                description=f"poco iteration {rebirth_data['next_iteration']}",
    ...
    def run_forever(self, hours_per_run: float = 5.5) -> dict:
        override = os.environ.get("poco_RUN_HOURS_OVERRIDE", "").strip()
        if override:
            try:
                hours_per_run = float(override)
            except Exception:
                pass
        max_runtime_raw = os.environ.get("poco_MAX_RUNTIME_HOURS", "5.0").strip()
        try:
        ...
        post_max = 180
        comment_min = 7
        comment_max = max(comment_min, _env_int("poco_COMMENT_INTERVAL_MAX_MINUTES", 15))
        mention_min = 1
        mention_max = max(mention_min, _env_int("poco_MENTION_INTERVAL_MAX_MINUTES", 5))
        notification_min = max(60, _env_int("poco_NOTIFICATION_INTERVAL_MIN_MINUTES", _env_int("poco_NOTIFICATION_INTERVAL_MINUTES", 120)))
        notification_max = max(notification_min, _env_int("poco_NOTIFICATION_INTERVAL_MAX_MINUTES", 180))
        now = time.time()
        next_post_at = now + (_env_random_int("poco_POST_INTERVAL_MIN_MINUTES", "poco_POST_INTERVAL_MAX_MINUTES", post_min, post_max) * 60)
        next_comment_at = now + (_env_random_int("poco_COMMENT_INTERVAL_MIN_MINUTES", "poco_COMMENT_INTERVAL_MAX_MINUTES", comment_min, comment_max) * 60)
        ...
        next_notification_at = now + (
            _env_random_int("poco_NOTIFICATION_INTERVAL_MIN_MINUTES", "poco_NOTIFICATION_INTERVAL_MAX_MINUTES", notification_min, notification_max) * 60
        )
        self._trace_runtime(
            "schedule",
            "ready",
            ...
            comments_between_posts=[
                _env_int("poco_COMMENTS_BETWEEN_POSTS_MIN", 4),
                _env_int("poco_COMMENTS_BETWEEN_POSTS_MAX", 8),
            ],
            comment_batch=[
                _env_int("poco_COMMENT_BATCH_MIN", 1),
                _env_int("poco_COMMENT_BATCH_MAX", 2),
            ],
        ...
        if _env_enabled("poco_BOOT_SEQUENCE_ONLY", "0"):
            if not self.last_posted_ok:
                raise RuntimeError("Initial poco cycle failed during boot validation")
        ...
        while time.time() < end_at:
            try:
                ...
                if now >= next_post_at:
                    ...
                    now = time.time()
                    next_post_at = now + (
                        _env_random_int("poco_POST_INTERVAL_MIN_MINUTES", "poco_POST_INTERVAL_MAX_MINUTES", post_min, post_max) * 60
                    )
                    next_comment_at = now + (
                        _env_random_int("poco_COMMENT_INTERVAL_MIN_MINUTES", "poco_COMMENT_INTERVAL_MAX_MINUTES", comment_min, comment_max) * 60
                    )
                    self._trace_runtime(
                        "schedule",
                        "post_rescheduled",
                ...
                if now >= next_mention_at:
                    ...
                    now = time.time()
                    next_mention_at = now + (
                        _env_random_int("poco_MENTION_INTERVAL_MIN_MINUTES", "poco_MENTION_INTERVAL_MAX_MINUTES", mention_min, mention_max) * 60
                    )
                if now >= next_notification_at:
                    ...
                    next_notification_at = now + (
                        _env_random_int(
                            "poco_NOTIFICATION_INTERVAL_MIN_MINUTES",
                            "poco_NOTIFICATION_INTERVAL_MAX_MINUTES",
                            notification_min,
                            notification_max,
                        )
                        * 60
